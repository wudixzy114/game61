...
namespace godot
{
	...
	namespace
	{
			...
			NativeTerrainTerms *r_terms = nullptr)
		{
			...
			double aridity = (1.0 - smooth_step(0.30, 0.58, base_moisture)) *const double rain_shadow_noise = ridged(
				(climate_warp_x + 2281.0) / (p_profile.continent_scale * 0.64),
				(climate_warp_z - 3167.0) / (p_profile.continent_scale * 0.64),
				p_profile.seed + 389,
				4);
			...
			const double micro = p_include_micro ? fbm(
													   p_x / 118.0,
													   p_z / 118.0,
													   p_profile.seed + 71,
													   4)
			...
			double p_exposure)
		{
			...
			const double snow = p_temperature < 0.22 ? smooth_step(p_profile.sea_level + 280.0, p_profile.sea_level + p_profile.height_scale * 0.92, p_height) : 0.0;
