...
{
    private static class TerrainWorldPlanQueryService
    {
        internal static TerrainWorldPointOfInterest[] GetPointsOfInterest(TerrainWorld world)
        {
            if (world._worldPlan is null)
            {
                return Array.Empty<TerrainWorldPointOfInterest>();
            }

            TerrainWorldPointOfInterest[] points = world._worldPlan.PointsOfInterest;
            return points.Length == 0
                ? Array.Empty<TerrainWorldPointOfInterest>()
                : (TerrainWorldPointOfInterest[])points.Clone();
        }

        internal static TerrainWorldRoute[] GetRoutes(TerrainWorld world)
        {
            if (world._worldPlan is null)
            {
            ...
            TerrainWorldRoute[] routes = world._worldPlan.Routes;
        ...
        internal static TerrainWorldPointOfInterestSummary[] QueryNearestPointsOfInterest(
            TerrainWorld world,
            Vector2 query,
            float radius,
            int maxResults,
        ...
        {
            if (world._worldPlan is null || maxResults <= 0)
            {
            ...
            foreach (TerrainWorldPointOfInterest point in world._worldPlan.PointsOfInterest)
            {
                ...
                float distanceSquared = point.WorldPosition.DistanceSquaredTo(query);
        ...
        internal static bool TryFindNearestPointOfInterest(
            TerrainWorld world,
            Vector2 query,
            float radius,
            TerrainPointOfInterestKind? kind,
            out TerrainWorldPointOfInterest point)
        {
            ...
            foreach (TerrainWorldPointOfInterest candidate in world._worldPlan.PointsOfInterest)
            {
                ...
                float distanceSquared = candidate.WorldPosition.DistanceSquaredTo(query);
        ...
        internal static TerrainWorldPointOfInterest[] QueryPointsOfInterest(
            TerrainWorld world,
            Rect2 worldBounds,
            TerrainPointOfInterestKind? kind)
        {
        ...
        internal static TerrainGameplayTagRegionSummary[] QueryGameplayTagRegions(
            TerrainWorld world,
            Rect2 worldBounds,
        ...
        internal static TerrainWorldRoute[] QueryRoutesNear(TerrainWorld world, Vector2 query, float radius)
        {
            if (world._worldPlan is null)
            {
                return Array.Empty<TerrainWorldRoute>();
            ...
            foreach (TerrainWorldRoute route in world._worldPlan.Routes)
            {
                if (DistanceSquaredToRoute(query, route) <= radiusSquared)
                {
        ...
        internal static TerrainWorldRouteSummary[] QueryRouteSummariesNear(
            TerrainWorld world,
            Vector2 query,
            float radius,
            int maxResults)
        {
            ...
            foreach (TerrainWorldRoute route in world._worldPlan.Routes)
            {
                float distanceSquared = DistanceSquaredToRoute(query, route);
                if (distanceSquared > radiusSquared)
                {
                    continue;
                }
        ...
        internal static TerrainRouteCorridorSample SampleRouteCorridor(TerrainWorld world, Vector2 query)
        {
            TerrainRouteCorridorIndex corridors = world._routeCorridors ?? TerrainRouteCorridorIndex.Empty;
            if (world._worldPlan is null || !corridors.HasSegments)
            {
                return TerrainRouteCorridorSample.None;
            }

            TerrainGenerationProfile profile = world.CurrentProfile;
            return corridors.Sample(query, CoordFromWorld(query, profile.ChunkSize));
        }
