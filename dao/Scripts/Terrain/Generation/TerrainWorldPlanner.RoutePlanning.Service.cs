...
{
    private static class RoutePlanningService
    {
        internal static TerrainWorldRoute[] BuildRoutes(
            ...
            while (remaining.Count > 0 && routes.Count < maxRoutes)
            {
                ...
                foreach (int from in connected)
                {
                    ...
                    foreach (int to in remaining)
                    {
                        ...
                        TerrainWorldRoute? route = RoutePathfinderService.TryBuildRoute(points[from], points[to], fields, profile, routeRules, resolution, cancellationToken);
                        if (route is null)
                        {
                            continue;
                        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (SecondaryRouteCandidate candidate in candidates)
            {
                ...
                testedCandidates++;
                TerrainWorldRoute? route = RoutePathfinderService.TryBuildRoute(from, to, fields, profile, routeRules, resolution, cancellationToken);
                if (route is null)
                {
                    continue;
                }
